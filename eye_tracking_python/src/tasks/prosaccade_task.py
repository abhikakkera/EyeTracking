...
Each trial:
  ...
  TARGET  (up to response_window_sec or target_duration_sec)
      → peripheral target appears left or right
      → participant should saccade TOWARD the target
      → first saccade onset detected → trial recorded
  INTER_TRIAL (inter_trial_interval_sec)
      → blank screen

Trial directions are randomised (equal left/right split).

Recorded per trial (SaccadeTrialRecord):
  • response latency
  • response direction (left/right/none)
  • direction_correct  (True if saccade was toward target)
  • response velocity and amplitude
...
_LEFT_X  = 0.5 - 0.35   # default left eccentricity (overridden by config)
_RIGHT_X = 0.5 + 0.35


class ProSaccadeTask(BaseTask):
    """
    Pro-saccade protocol: look TOWARD the peripheral target.
    """

    TASK_NAME = "Pro-Saccade Task"
    ...
    def __init__(self, config: AppConfig) -> None:
        ...
        # Planned trial list (SaccadeDirection.LEFT / RIGHT)
        ...
        self._gaze_at_target_onset: float = 0.5  # gaze_x when target appeared
        self._vel_history: Deque[float] = deque(maxlen=10)

        # Completed trials
        self._completed: List[SaccadeTrialRecord] = []
        self._pending_trial: Optional[SaccadeTrialRecord] = None  # just completed
    ...
    def on_tracking_frame(self, record: FrameRecord) -> None:
        """Advance state machine; detect saccade onset."""
        self._just_completed = False
        ...
        if self._phase == TaskPhase.WAITING:
            ...
            if record.face_detected and not record.blink_detected:
                ...
                if onset:
                    self._record_response(record, t)
                    return

            # Timeout: response window expired
    ...
    def current_context(self) -> TaskContext:
        ...
        return TaskContext(
            ...
            frame_number   = 0,    # filled by TaskRunner
            timestamp_sec  = 0.0,  # filled by TaskRunner
    ...
    def _record_response(self, record: FrameRecord, t: float) -> None:
        """Saccade onset detected — classify direction and record."""
        ...
        latency_ms = (t - self._target_onset_sec) * 1000.0
        ...
        trial = SaccadeTrialRecord(
            ...
            response_latency_ms = max(0.0, latency_ms),
